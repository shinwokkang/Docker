...
### 💡 [Q&A] 내 컴퓨터는 Windows이고, 동료 B는 Linux인데 도커가 어떻게 활용되는 건가요?
이 부분이 바로 도커가 전 세계 인프라를 지배하게 된 핵심 비결인 **"Build once, Run anywhere (한 번 빌드하면 어디서든 돌아간다)"**의 마법입니다. 

수강생님의 컴퓨터는 겉보기엔 Windows입니다. 하지만 앞서 설명한 **WSL2 기술 덕분에 도커는 보이지 않는 곳에서 진짜 리눅스 커널을 돌리고 있습니다.**
따라서 수강생님이 Windows 터미널에서 `docker build`를 치면, 사실은 Windows용 프로그램이 아니라 **"완벽한 100% 리눅스 컨테이너 이미지"**가 구워집니다. 이 이미지 안에는 리눅스 OS(예: Ubuntu)와 리눅스용 파이썬, 그리고 수강생님의 소스코드가 담겨 있습니다.

1. **Windows 개발자 (수강생님):** Windows 환경이지만 WSL2를 통해 완벽한 리눅스 컨테이너를 빌드하고 테스트합니다. 그리고 이 이미지를 도커 허브(Docker Hub)에 올립니다.
2. **Linux 운영자 (동료 B):** 동료 B는 리눅스 서버에서 수강생님이 올린 이미지를 그대로 다운받아(`docker pull`) 실행(`docker run`)합니다. 

만약 도커가 없었다면 어땠을까요? 수강생님 컴퓨터에서는 경로가 `C:\app`이고 동료 컴퓨터는 `/usr/src/app`이어서 에러가 터지는 이른바 **"내 컴퓨터에선 잘 되는데요?"** 현상이 100% 발생합니다. 
하지만 도커를 사용하면, 수강생님의 Windows 노트북 내부(WSL2)나 동료 B의 실제 리눅스 서버나 **결국 똑같은 "리눅스 커널 위에서 똑같은 컨테이너"를 돌리는 격**이 되므로 완벽하게 동일하게 작동합니다! 이것이 도커를 활용한 크로스 플랫폼 협업의 본질입니다.
...
1. **Windows Server Core**
   ...
   * **💡 이해를 돕는 실무 예시:** A병원에서 2012년에 만든 '환자 관리 시스템'이 낡은 Windows Server 2012와 구형 ASP.NET 위에서 돌고 있습니다. 이 코드를 최신식으로 다시 짜려면 수십억 원이 듭니다. 이때 이 시스템 전체를 뜯어내어 거대한 `Windows Server Core` 이미지 안에 통째로 담아 AWS나 Azure 클라우드로 그대로 옮겨버립니다. 코드는 1줄도 안 바꿨지만, 클라우드의 장점(자동 스케일링, 복구 등)을 누릴 수 있게 됩니다.
2. **Nano Server**
   ...
   * **💡 이해를 돕는 실무 예시:** B스타트업이 아주 가볍고 빠른 '실시간 채팅 알림 서버'를 최신 기술인 `.NET 8`을 사용해 새로 개발했습니다. 이 서버는 화면(UI)도 필요 없고 구형 윈도우 기능도 전혀 필요 없습니다. 오직 코드만 쌩쌩 돌아가면 됩니다. 이때 `Nano Server` 베이스 이미지를 쓰면, 마치 리눅스의 알파인(Alpine) 리눅스처럼 불필요한 덩치를 쏙 뺀 수십 MB 크기의 초경량 컨테이너를 수천 개씩 빠르게 찍어낼 수 있습니다.
...
* **💡 이해를 돕는 실무 예시:** 우리 회사의 내부 데이터센터에는 **정확히 똑같은 Windows Server 2022 운영체제가 깔린 서버 100대**가 있습니다. 이곳에 올릴 컨테이너 이미지들도 전부 Windows Server 2022 기반으로 통일해서 만들었습니다. 호스트와 컨테이너의 버전이 완벽히 일치하므로, 굳이 무겁게 가상머신을 거칠 필요가 없습니다. 하나의 커널 위에서 컨테이너들이 리눅스처럼 쌩쌩 돌아가도록 `Process Isolation`을 써서 최고 성능을 냅니다.
...
* **💡 이해를 돕는 실무 예시:** 마이크로소프트 Azure 클라우드에서 '서버리스 컨테이너 서비스'를 운영합니다. A고객은 Windows 2016 기반의 악성코드 분석 컨테이너를 올리고, B고객은 Windows 2022 기반의 웹 서버를 올립니다. 호스트 서버 하나에 버전도 제각각이고 서로 믿을 수 없는 고객들의 컨테이너가 뒤섞여 있습니다. 만약 A고객의 악성코드가 탈옥해서 서버 커널을 망치면 B고객까지 피해를 봅니다. 그래서 Azure는 각 컨테이너를 **보이지 않는 투명하고 단단한 티타늄 박스(마이크로 가상머신)**에 가두어 실행시킵니다. 이것이 `Hyper-V Isolation`입니다. 안전하고 버전 제약도 없습니다.
