...
```bash
# 1. CentOS라는 리눅스 운영체제 컨테이너 안으로 접속합니다!
# bash라는 명령어를 추가하여, 컨테이너 안에서 명령어를 칠 수 있는 상태로 만듭니다.
docker run -it centos bash
```
*❗ 터미널 앞부분의 글자가 `shinwookkang@...` 에서 갑자기 `root@어쩌구저쩌구#` 로 바뀌었나요? 축하합니다! 지금 여러분의 터미널은 내 Mac이 아니라, 격리된 CentOS 컨테이너 내부입니다!*
...
cat /etc/*release*
```
*(`CentOS Linux release ...` 라는 문구가 보일 것입니다.)*

```bash
# 3. 방 구경을 다 했으니 밖으로 나갑시다.
exit
```
*(다시 원래의 `shinwookkang@...` 터미널로 돌아오며, 주인이 나가버린 컨테이너는 할 일이 없어져서 스스로 종료됩니다.)*
...
# (docker ps에 나온 컨테이너 ID의 앞 3자리 정도만 적어주세요!)
docker stop 컨테이너ID
...
docker images
```
*(CentOS는 200MB가 넘지만, busybox는 겨우 **1MB** 남짓인 것을 확인하실 수 있습니다!)*
